...
freqtrade download-data \
    -c user_data/binance_futures_MultiWmaMacd.json \
    --timerange 20230101- \
...
"""
freqtrade hyperopt \
    --strategy MultiWmaMacd \
    --config user_data/binance_futures_MultiWmaMacd.json \
    --timeframe 1h \
...
class MultiWmaMacd(IStrategy):
    ...
    # Strategy parameters
    wma_rolling_window = CategoricalParameter([4, 6], default=4, space="buy")
    macd_rolling_window = CategoricalParameter([13, 10], default=13, space="buy")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe["wma13"].rolling(window=self.wma_rolling_window.value).apply(
                ...
                (dataframe["wma21"].rolling(window=self.wma_rolling_window.value).apply(
                    lambda x: any(qtpylib.crossed_above(x, dataframe["wma34"].iloc[x.index[0]:x.index[-1]+1])))) &

...
# ==========================================
# Profitable Williams %R Strategy for Python Trading Bots — Full Backtest
# https://youtu.be/rm09Ce8QsIY
# ==========================================
...
freqtrade download-data \
    -c user_data/binance_futures_WILLR_RSI_MACD.json \
    --timerange 20230101- \
...
"""
freqtrade hyperopt \
    --strategy WILLR_RSI_MACD \
    --config user_data/binance_futures_WILLR_RSI_MACD.json \
    --timeframe 1h \
...
class WILLR_RSI_MACD(IStrategy):
    ...
    willr_threshold = CategoricalParameter([45, 40], default=45, space="buy")
    willr_rolling_window = CategoricalParameter([5, 10, 15], default=10, space="buy")

    rsi_threshold = CategoricalParameter([50, 55, 60], default=55, space="buy")
    rsi_macd_rolling_window = CategoricalParameter([2, 4, 6], default=4, space="buy")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:        
        ...
        dataframe.loc[
            (   
                ...
                (dataframe["willr"].rolling(window=self.willr_rolling_window.value).min() < -80) &
                (dataframe["willr"] > (-1 * self.willr_threshold.value)) &
        ...
        dataframe.loc[
            (
                ...
                (dataframe["willr"].rolling(window=self.willr_rolling_window.value).max() > -20) &
                (dataframe["willr"] < (self.willr_threshold.value - 100)) &
